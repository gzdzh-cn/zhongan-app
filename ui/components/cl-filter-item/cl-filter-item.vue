				...
				<template v-if="dropdown.list">
					...
					<template v-if="dropdown.list.length > 0">
						...
						<scroll-view class="cl-filter-item__dropdown-box__list" scroll-y>
							<view class="cl-filter-item__dropdown-box__item" :class="[
								item.checked ? 'is-checked' : '',
								item.disabled ? 'is-disabled' : '',
							]" :style="{
	width: itemWidth,
}" v-for="(item, index) in dropdown.list" :key="index" @tap.stop="checkItem(item)">
...
export default defineComponent({
	...
	props: {
		...
		type: {
			type: String as PropType<"dropdown" | "order" | "switch" | "pop">,
			default: "switch",
		},
		label: String,
		disabled: Boolean,
